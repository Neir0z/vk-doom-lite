  ...
  constructor(canvas) {
   ...
   render(time, player, wallTex) {
    ...
    for (let x = 0; x < width; x++) {
      const rayAngle = (player.angle - RENDER.fov/2) + (x/width) * RENDER.fov;
      ...
      else { stepX = 1; sideDistX = (mapX+1 - player.x/RENDER.mapScale)*deltaDistX; }
      if (eyeY < 0) { stepY = -1; sideDistY = (player.y/RENDER.mapScale - mapY)*deltaDistY; }
      else { stepY = 1; sideDistY = (mapY+1 - player.y/RENDER.mapScale)*deltaDistY; }
      ...
      if (wallTex) {
        const texX = Math.floor((side === 0 ? player.y : player.x) * 0.5) % 64;
        ...
        const shade = Math.max(0.2, 1 - dist/RENDER.maxDepth);
        ctx.fillStyle = `rgb(${Math.floor(110*shade)},${Math.floor(110*shade)},${Math.floor(150*shade)})`;
